...
from model.base.transformer import *


class GLQSCA(nn.Module):

    def __init__(self, backbone, pretrained_path, use_original_imgsize):
        super(GLQSCA, self).__init__()
        ...
        self.model = GLQSCA_model(in_channels=self.feat_channels, stack_ids=self.stack_ids)
...
class GLQSCA_model(nn.Module):
    def __init__(self, in_channels, stack_ids):
        super(GLQSCA_model, self).__init__()

        self.stack_ids = stack_ids

        # GLQSCA blocks
        self.GLQSCA_blocks = nn.ModuleList()
        self.pe = nn.ModuleList()
        for inch in in_channels[1:]:
            self.GLQSCA_blocks.append(MultiHeadedAttention_v3(h=8, d_model=inch, dropout=0.5))
    ...
    def forward(self, query_feats, support_feats, support_mask, nshot=1):
        ...
        for idx, query_feat in enumerate(query_feats):
            ...
            # GLQSCA blocks forward
            if idx < self.stack_ids[1]:
                coarse_mask = self.GLQSCA_blocks[0](self.pe[0](query), self.pe[0](support_feat), mask)
            elif idx < self.stack_ids[2]:
                coarse_mask = self.GLQSCA_blocks[1](self.pe[1](query), self.pe[1](support_feat), mask)
            else:
                coarse_mask = self.GLQSCA_blocks[2](self.pe[2](query), self.pe[2](support_feat), mask)
